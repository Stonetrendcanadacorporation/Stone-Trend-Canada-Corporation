#!/usr/bin/env python3
"""Generate individual quartzite slab detail pages."""
import json
import html
from pathlib import Path

DATA = Path(__file__).parent.parent / "data" / "olympia-quartzite.json"
QUARTZITE_DIR = Path(__file__).parent.parent / "quartzite"
...
<head>
  ...
  <title>{html.escape(name)} | Quartzite Slabs | Stone Trend</title>
  <meta name="description" content="{html.escape(name)} quartzite slab. {html.escape(desc[:120])}..." />
...
{spec_rows}
        ...
        <p class="small" style="margin-top: 2rem;">
          <a href="../quartzite-slabs.html">← Back to quartzite catalogue</a>
        </p>
      </div>
    </section>
  </main>
...
def main():
    ...
    QUARTZITE_DIR.mkdir(exist_ok=True)
    for s in slabs:
        slug = s.get("slug", "")
        if not slug:
            continue
        with open(QUARTZITE_DIR / f"{slug}.html", "w") as f:
            f.write(render_page(s))
    print(f"Generated {len(slabs)} quartzite detail pages in {QUARTZITE_DIR}")
