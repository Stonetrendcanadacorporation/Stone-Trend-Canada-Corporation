#!/usr/bin/env python3
"""Fetch travertine slab data from olympiatile.com slabs-travertine-series."""
import json
import re
import ssl
...
URL = "https://www.olympiatile.com/en/slabs-travertine-series.html"
DATA = Path(__file__).parent.parent / "data" / "olympia-travertine.json"

SKU_WORDS = {
    "wht": "White", "wh": "White", "blk": "Black", "gr": "Grey", "gry": "Grey",
    "crem": "Cream", "crm": "Crema", "br": "Brown", "bl": "Blue",
    "pl": "Polished", "hn": "Honed", "bianco": "Bianco", "nero": "Nero",
    "tan": "Tan", "brown": "Brown", "blu": "Blue", "bia": "Bianco",
    "grig": "Grigio", "sil": "Silver", "noce": "Noce", "gold": "Gold",
    "ivory": "Ivory", "walnut": "Walnut", "chocolate": "Chocolate",
    "tr": "Travertine", "trl": "Travertine Light", "trav": "Travertine", "t": "Travertine",
    "jurass": "Jurassico", "navona": "Navona", "rom": "Romano",
    "red": "Red", "cchd": "C&C", "cc": "C&C", "vc": "Vein Cut",
    "cl": "Classic", "vc2cm": "", "cchd2cm": "",
}
...
def sku_to_name(sku):
    if not sku:
        return None
    s = re.sub(r"^[A-Z]{2}\.[A-Z]{2}\.", "", sku.upper())
    s = re.sub(r"\.?[23]CM$", "", s, flags=re.I)
    s = re.sub(r"VC2CM|CCHD2CM", "", s, flags=re.I)
...
def main():
    print("Fetching olympiatile.com slabs-travertine-series...")
    html = fetch_page()
    ...
    id_to_sku = {}
    for m in re.findall(r'"(\d{5})":"([A-Z]{2}\.[^"]+)"', html):
        pid, sku = m
    ...
    print("Found", len(ids_2cm), "products in 2CM,", len(ids_3cm), "in 3CM")
    print("Wrote", len(slabs), "travertine slabs to", DATA)
