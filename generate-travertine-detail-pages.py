#!/usr/bin/env python3
"""Generate individual travertine slab detail pages."""
import json
import html
from pathlib import Path

DATA = Path(__file__).parent.parent / "data" / "olympia-travertine.json"
TRAVERTINE_DIR = Path(__file__).parent.parent / "travertine"
        ...
        <ul>
          ...
          <li class="has-dropdown">
            ...
            <div class="dropdown">
              ...
              <a href="../travertine-slabs.html">Travertine</a>
...
def get_description(s):
    ...
    if len(colors) >= 2 and "multicolour" not in colors:
        ...
        color_str = color_str + " travertine"
    return f"{color_str.capitalize()} with characteristic movement and texture. Available in {thickness}. Suitable for countertops, vanities, walls, and feature applications. Seal before grouting."
...
def render_page(s):
    ...
    # Use absolute URLs as-is; relative paths need ../ from travertine/
...
<head>
  ...
  <title>{html.escape(name)} | Travertine Slabs | Stone Trend</title>
  <meta name="description" content="{html.escape(name)} travertine slab. {html.escape(desc[:120])}..." />
...
{spec_rows}
        ...
        <p class="small" style="margin-top: 2rem;">
          <a href="../travertine-slabs.html">← Back to travertine catalogue</a>
        </p>
      </div>
    </section>
  </main>
...
def main():
    ...
    TRAVERTINE_DIR.mkdir(exist_ok=True)
    for s in slabs:
        slug = s.get("slug", "")
        if not slug:
            continue
        with open(TRAVERTINE_DIR / f"{slug}.html", "w") as f:
            f.write(render_page(s))
    print(f"Generated {len(slabs)} travertine detail pages in {TRAVERTINE_DIR}")
