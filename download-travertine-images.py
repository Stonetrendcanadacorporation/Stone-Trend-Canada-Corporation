#!/usr/bin/env python3
"""Download travertine slab images to images/travertine/{slug}.jpg, update JSON."""
import json
import ssl
...
IMG_DIR = Path(__file__).parent.parent / "images" / "travertine"
SIZE = 400
...
def main():
    ...
    for s in slabs:
        ...
        try:
            ...
            rel = f"images/travertine/{slug}.jpg"
            s["img_small"] = rel
            s["img_large"] = rel
            ok += 1
    ...
    print(f"Downloaded/saved {ok} travertine images to {IMG_DIR} (uniform squares {SIZE}x{SIZE})")
    if err:
