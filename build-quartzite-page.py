#!/usr/bin/env python3
"""Build quartzite-slabs.html from quartzite JSON data."""
import json
import html
import re
...
OUT = Path(__file__).parent.parent / "quartzite-slabs.html"
...
def main():
    ...
    for i, s in enumerate(slabs):
        ...
        slug = s.get("slug", "")
        detail_url = f"quartzite/{slug}.html" if slug else ""
        cards.append(
            f'          <article class="gallery-item quartzite-slab-card" data-animate="fade-up" data-animate-delay="{delay}" '
            f'data-category="{html.escape(category)}" data-colors="{colors_attr}" data-thickness="{html.escape(thickness_attr)}" data-format="slab">\n'
            f'            <a href="{html.escape(detail_url)}" class="quartzite-slab-image-link" aria-label="View {html.escape(name)} details">\n'
            f'              <img src="{html.escape(img)}" alt="{html.escape(name)}" class="quartzite-slab-thumb" loading="lazy" decoding="async" fetchpriority="low" width="400" height="400">\n'
            f"            </a>\n"
            f'            <div class="gallery-info">\n'
            f'              <h3><a href="{html.escape(detail_url)}" class="quartzite-slab-title-link">{html.escape(name)}</a></h3>\n'
            f'              <p class="quartzite-slab-thickness-badge" aria-label="Thickness">{html.escape(thickness_label)}</p>\n'
            f"              <p>{desc}</p>\n"
            f'              <a href="{html.escape(detail_url)}" class="quartzite-view-details">View details <span aria-hidden="true">→</span></a>\n'
            f"            </div>\n"
            f"          </article>"
        )
    ...
    pattern = r'(<div class="gallery-grid"[^>]*id="quartzite-gallery-grid"[^>]*>)(.*?)(</div>\s*\n\s*<p class="small")'
    replacement = rf"\1\n{cards_html}\n        \3"
    ...
    print(f"Updated {OUT} with {len(slabs)} quartzite slabs")
