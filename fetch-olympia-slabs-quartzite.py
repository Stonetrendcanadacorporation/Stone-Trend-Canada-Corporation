#!/usr/bin/env python3
"""Fetch quartzite slab data from olympiatile.com slabs-quartzite-series and build data/olympia-quartzite.json."""
import json
import re
import ssl
...
URL = "https://www.olympiatile.com/en/slabs-quartzite-series.html"
DATA = Path(__file__).parent.parent / "data" / "olympia-quartzite.json"

# SKU abbreviation -> full word (quartzite slab names)
SKU_WORDS = {
    ...
    "mb": "MB", "pol": "Polished", "b": "Bianco", "grig": "Grigio",
    ...
    "mac": "Macaubas", "calac": "Calacatta", "venat": "Venato", "stat": "Statuario",
    "sup": "Super", "del": "Delicato", "fant": "Fantasy", "arab": "Arabescato",
}
...
def sku_to_name(sku):
    """Convert SKU to display name (quartzite slab)."""
    if not sku:
        return None
    # Strip common prefixes: GQ.SB., QZ.SB., etc. and thickness suffix
...
def main():
    print("Fetching olympiatile.com slabs-quartzite-series...")
    html = fetch_page()
    ...
    # Product ID -> SKU (GQ., QZ., or similar for quartzite)
    id_to_sku = {}
    ...
    print(f"Wrote {len(slabs)} quartzite slabs to {DATA}")
